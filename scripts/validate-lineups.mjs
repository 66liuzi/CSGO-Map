...
for (const [i, l] of lineups.entries()) {
  ...
  for (const key of ['id', 'side', 'startLocation', 'targetLocation', 'grenadeType', 'throwMethod', 'description', 'image', 'zone']) {
  ...
  const combo = [l.side, l.startLocation, l.targetLocation, l.grenadeType, l.throwMethod].join('|')
...
console.log(`[validate] 共 ${lineups.length} 条点位（其中示例 ${samples} 条）`)
if (warnings.length) {
  console.log('\n提醒：')
